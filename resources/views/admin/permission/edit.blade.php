...
@section('content')
    ...
    @include('admin.partials.content-header',['name'=>'permission','key'=>'Edit'])
    ...
    <!-- Main content -->
        <div class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <form method="post" action="{{ route('permission.update',['id'=>$permissionItem->id]) }}" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label>Chọn tên module</label>
                                <input type="text" class="form-control" name="module_parent" value="{{$permissionItem->name}}">
                            </div>
                            <div class="form-group">
                                <div class="row">
                                    @foreach($permissionParent as $permissionItem)
                                        @foreach($permissionItem->permissionChildren as $permissionChildrenItem)
                                            <div class="col-md-3">
                                                <label>
                                                    <input type="checkbox"
                                                           {{$permissionChild->contains('id',$permissionChildrenItem->id) ? 'checked':''}}
